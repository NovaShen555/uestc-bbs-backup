...
function renderContent(content, attachMap) {
  ...
  html = html.replace(/!\[(\d+)\]\(([a-z])\)/g, (match, emojiId, prefix) => {
    if (prefix === 'a') {
      const pack = EMOJI_PACKS.find(p => p.path === 'alu');
      if (pack) {
        const item = pack.items.find(([id]) => id == emojiId);
        if (item) {
          return `<img src="${BBS_BASE}/static/image/smiley/${pack.path}/${item[1]}" alt="![${emojiId}](${prefix})" class="emoji">`;
        }
      }
